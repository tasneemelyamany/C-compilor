...
int *text, *oldtext, *stack;
char *data;
int *sp, *pc, *bp, ac , ins_cycle;
int *idmain;
...
void next() {
    ...
    while ((token = *src)) {
        ...
        if (token == '\n') {
            if (assembly) {
                // print compile info
                char placeholder[11] = "%lld: %.*s";
                printf(placeholder, line, src - old_src, old_src);
                old_src = src;

                while (oldtext < text) {
                    printf("%8.4s", & "LEA ,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,"
                                      "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                                      "OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,EXIT"[*++oldtext * 5]);
...
int eval() { 
    ...
    while (1) {
        ...
        if (op == IMM)       {ac = *pc++;}                                     // load immediate value to ac
        else if (op == LC)   {ac = *(char *)ac;}                               // load character to ac, address in ac
        else if (op == LI)   {ac = *(int *)ac;}                                // load integer to ac, address in ac
        else if (op == SC)   {ac = *(char *)*sp++ = ac;}                       // save character to address, value in ac, address on stack
        else if (op == SI)   {*(int *)*sp++ = ac;}                             // save integer to address, value in ac, address on stack
        else if (op == PUSH) {*--sp = ac;}                                     // push the value of ax onto the stack
        else if (op == JMP)  {pc = (int *)*pc;}                                // jump to the address
        else if (op == CALL) {*--sp = (int)(pc+1); pc = (int *)*pc;}    
        //else if (op == RET)  {pc = (int *)*sp++;}
        else if (op == JZ)   {pc = ac ? pc + 1 : (int *)*pc;}                   // jump if ac is zero"false"
        else if (op == JNZ)  {pc = ac ? (int *)*pc : pc + 1;}                   // jump if ac is not 0"true"
        else if (op == ENT)  {*--sp = (int)bp; bp = sp; sp = sp - *pc++;}      // make new stack frame
        else if (op == ADJ)  {sp = sp + *pc++;}                                // add esp, <size>
        else if (op == LEV)  {sp = bp; bp = (int *)*sp++; pc = (int *)*sp++;}  // restore call frame and PC
        else if (op == LEA)  {ac = (int)(bp + *pc++);}                         // load address for arguments. //special ADD
        else if (op == OR)  ac = *sp++ | ac;
        else if (op == XOR) ac = *sp++ ^ ac;
        else if (op == AND) ac = *sp++ & ac;
        else if (op == EQ)  ac = *sp++ == ac;
        else if (op == NE)  ac = *sp++ != ac;
        else if (op == LT)  ac = *sp++ < ac;
        else if (op == LE)  ac = *sp++ <= ac;
        else if (op == GT)  ac = *sp++ >  ac;
        else if (op == GE)  ac = *sp++ >= ac;
        else if (op == SHL) ac = *sp++ << ac;
        else if (op == SHR) ac = *sp++ >> ac;
        else if (op == ADD) ac = *sp++ + ac;
        else if (op == SUB) ac = *sp++ - ac;
        else if (op == MUL) ac = *sp++ * ac;
        else if (op == DIV) ac = *sp++ / ac;
        else if (op == MOD) ac = *sp++ % ac;
        else if (op == EXIT) { printf("exit(%lld)", *sp); return *sp;}
        else if (op == OPEN) { ac = open((char *)sp[1], sp[0]); }
        else if (op == CLOS) { ac= close(*sp);}
        else if (op == READ) { ac = read(sp[2], (char *)sp[1], *sp); }
        else if (op == PRTF) { tmp = sp + pc[1]; ac = printf((char *)tmp[-1], tmp[-2], tmp[-3], tmp[-4], tmp[-5], tmp[-6]); }
        else if (op == MALC) { ac = (int)malloc(*sp);}
        else if (op == MSET) { ac = (int)memset((char *)sp[2], sp[1], *sp);}
        else if (op == MCMP) { ac = memcmp((char *)sp[2], (char *)sp[1], *sp);}
        else {
